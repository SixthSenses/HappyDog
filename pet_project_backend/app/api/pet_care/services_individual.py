# app/api/pet_care/services_individual.py
import uuid
import logging
...
from app.models.pet_care_log import (
    PetCareLog, FoodLog, WaterLog, PoopLog, ActivityLog, 
    VomitLog, WeightLog, MedicationLog, SymptomsLog
)
from app.api.breeds.services import BreedService

logger = logging.getLogger(__name__)

class IndividualPetCareService:
    """
    개별 리소스 기반의 펫케어 서비스 클래스
    각 로그를 독립적으로 관리하고 Firestore ArrayUnion/ArrayRemove를 활용
    """
    ...
    def get_daily_log(self, pet_id: str, user_id: str, log_date: date) -> Optional[Dict[str, Any]]:
        """특정 날짜의 모든 기록을 조회합니다."""
        try:
            if not self._verify_pet_ownership(pet_id, user_id):
                raise PermissionError(f"펫 {pet_id}에 대한 접근 권한이 없습니다.")
            ...
            logger.error(f"일일 기록 조회 실패 ({pet_id}, {log_date}): {e}")
            raise
    
    def calculate_recommendations(self, pet_id: str) -> Dict[str, Any]:
        """권장 칼로리 및 음수량을 계산합니다. (기존 로직 재사용)"""
        try:
            # 펫 정보 조회
            pet_doc = self.pets_collection.document(pet_id).get()
            if not pet_doc.exists:
